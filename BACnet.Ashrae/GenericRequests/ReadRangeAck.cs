...
{
    public partial class ReadRangeAck<TData> : IComplexAck
    {
        ...
        public ReadOnlyArray<TData> ItemData { get; private set; }

        public Option<uint> FirstSequenceNumber { get; private set; }

        public ReadRangeAck(ObjectId objectIdentifier, PropertyIdentifier propertyIdentifier, Option<uint> propertyArrayIndex, ResultFlags resultFlags, uint itemCount, ReadOnlyArray<TData> itemData, Option<uint> firstSequenceNumber)
        {
        ...
        public static readonly ISchema Schema = new SequenceSchema(false,
            ...
            new FieldSchema("ItemData", 5, Value<ReadOnlyArray<TData>>.Schema),
            new FieldSchema("FirstSequenceNumber", 6, Value<Option<uint>>.Schema));

        public static ReadRangeAck<TData> Load(IValueStream stream)
        {
            ...
            var itemData = Value<ReadOnlyArray<TData>>.Load(stream);
            var firstSequenceNumber = Value<Option<uint>>.Load(stream);
            stream.LeaveSequence();
            return new ReadRangeAck<TData>(objectIdentifier, propertyIdentifier, propertyArrayIndex, resultFlags, itemCount, itemData, firstSequenceNumber);
        }
